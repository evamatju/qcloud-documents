...
3. **将明文串转为最终签名**（以部分 Java 代码为例）
	1. 用已获取的 SecretKey 对明文串 original 进行 [HMAC-SHA1](https://www.ietf.org/rfc/rfc2104.txt) 加密，得到 signatureTmp，旧版签名直接将不可打印的 signatureTmp 与 original 的字节数组拼接再进行编码；新版签名为方便调试，将 signatureTmp 转成可打印的字符串，再进行拼接和编码，同时也会兼容旧版签名：
	```java
   ...
   mac.init(secretKey);
	 ...
	 String signatureStr = getHexString(signatureTmp);
	```
	>?**signatureTmp 是使用 UTF-8 编码、通过 HMAC-SHA1 加密出来的字节数组，signatureStr 是 signatureTmp 对应的十六进制字符串**。
	2. 将 signatureStr 与明文串 original 进行合并，然后把合并后的字符串使用 UTF-8 编码成字节数组，最后把结果进行 [Base64](https://tools.ietf.org/html/rfc4648) 编码，得到最终签名 signature：
```java
String signature = base64Encode((signatureStr+original).getBytes("utf8"));
```
>?**getHexString 是将二进制数组转换成十六进制字符串的方法，base64Encode 是 Base64 编码数组的方法，详细请参见 [Java 签名示例代码](https://cloud.tencent.com/document/product/266/10638#java-.E7.AD.BE.E5.90.8D.E7.A4.BA.E4.BE.8B)**。
